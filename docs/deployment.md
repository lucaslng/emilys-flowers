...
| `OIDC_ISSUER` | `src/lib/admin-auth.ts` | Provider issuer URL (e.g. `https://accounts.example.com`); discovery doc fetched at `{issuer}/.well-known/openid-configuration`. Must exactly match the issuer the provider reports in its discovery document — on mismatch, admin auth fails closed. Missing → admin page shows a config error. |
