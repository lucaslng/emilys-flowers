...
- **`open-next.config.ts`** is the OpenNext adapter config. On `feature/payload-cms` it wires ISR storage: the incremental cache is a custom **Backblaze B2** override (`b2-incremental-cache.ts`, aws4fetch → B2's S3 endpoint, `B2_*` env vars) and the tag cache is D1 (`NEXT_TAG_CACHE_D1`). On `main`, `defineCloudflareConfig({})` defaults apply (dummy caches, no ISR persistence).
...
- **Products come from Payload CMS** (D1 database), read at request time by the server-only module `src/lib/payload-catalog.ts` — the Stripe catalog fetch is gone on this branch. It queries the `products` collection via `getPayload` and maps docs onto the `Product` shape: slug stored on the doc (unique), `price` in integer cents, `category` select, `tags` array, `featured`/`featuredOrder`, `inStock`, `flowerType`/`color`, optional `media` uploads. Queries are per-request (no memoization) so on-demand revalidation (`revalidateTag('products', 'max')`) sees fresh data; only the Payload instance is memoized per request with React `cache`. `getPayload` + `@payload-config` are lazy-imported — a static config import crashes `bun test` via a Lexical circular-init. During `next build` D1 is unmigrated/unseeded, so `fetchProducts` degrades to an empty catalog when `NEXT_PHASE === 'phase-production-build'` and pages render on first request. Helpers: `getAllProducts`, `getProductBySlug`, `getProductsByCategory`, `getFeaturedProducts`; pure `mapPayloadProduct` + `slugify` exported for tests. Client-safe pure helpers (`getPriceRange`, `getFlowerTypes`, `getFlowerColors`, `formatLabel`) live in `src/lib/product-utils.ts`. Images are CMS media URLs (UploadThing `utfs.io`) when attached, else local per-category SVG placeholders (`/placeholders/flower.svg`, `/placeholders/bouquet.svg`) rendered through the `ProductImage` component (`src/components/shop/ProductImage.tsx`), which falls back to the category SVG on image-load error. `next.config.ts` `images.remotePatterns` is empty (no remote hosts; `utfs.io` pending Phase 3 of the migration). `STRIPE_SECRET_KEY` is no longer needed at build time — only at runtime by the checkout route (see the Stripe section).
