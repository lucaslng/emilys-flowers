...
Standard checkout at the repo root — **not** `.slim/worktrees/`; the repo root itself is registered as the OpenChamber project. Branches keep the required prefixes (`feature/`, `bugfix/`, `docs/`), based off `main`; name branches explicitly. New lanes bootstrap automatically via the project's `setup-worktree` config (`bun install` + copy `.env` from the repo root). Integration uses OpenChamber's **Integrate** (cherry-pick onto the chosen target — source branch survives, delete it explicitly afterward); do all lane work inside the worktree, not the main checkout.
