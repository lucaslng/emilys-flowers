...
- **Tailwind v4, CSS-first config**: there is no `tailwind.config.js`. Theme tokens are declared with `@theme inline` in `src/app/globals.css`. Custom color tokens: `background`, `foreground`, `surface`, `blush`, `lavender`, `rose`, `border`, `muted`. Fonts map to `--font-sans` (Inter) / `--font-serif` (Playfair Display). The contrast-critical values are: muted text `#7A6868`, text/hover rose `#9E5E5E` (both 4.5:1 on the light surfaces), and non-text rose (borders/underlines) `#B16E6E` (3:1). Don't lighten these back without re-checking WCAG contrast.
