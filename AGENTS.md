...
This repo is a **standard checkout** with **OpenChamber-managed worktree lanes** — not `.slim/worktrees/` and not hand-run `git worktree add`. When starting worktree operations, load the project skill from `.agents/skills/worktrees/SKILL.md`; the global worktrees skill describes a different convention.
<!-- END:worktree-skill -->
...
Standard checkout at the repo root — **not** `.slim/worktrees/`; the repo root itself is registered as the OpenChamber project. Worktree lanes are created and managed through **OpenChamber** — new lanes live under `~/.local/share/opencode/worktree/<repo-hash>/<name>/` with folder names slugged from the branch (`feature/foo` → `feature-foo`). Branches keep the required prefixes (`feature/`, `bugfix/`, `docs/`), based off `main`; name branches explicitly (never OpenChamber's auto `openchamber/<folder>`). New lanes bootstrap automatically via the project's `setup-worktree` config (`bun install` + copy `.env` from the repo root). Integration uses OpenChamber's **Integrate** (cherry-pick onto the chosen target — source branch survives, delete it explicitly afterward); do all lane work inside the worktree, not the main checkout. See `.agents/skills/worktrees/SKILL.md` for the full protocol.
...
- `cloudflare-env.d.ts` is **committed** and included in typecheck. It is generated by `bun run cf-typegen` (`wrangler types --include-runtime false`). Env-var members are typed as **required** (present at worker runtime); tests manipulate env via `unsetEnv` (`Reflect.deleteProperty`) from `src/lib/__tests__/env-helpers.ts`, never `delete process.env.X`. Runtime types stay off — workerd globals collide with lib.dom/@types/node/bun-types — so binding types (`Fetcher`, `RateLimit`, `ImagesBinding`) resolve only via `skipLibCheck`. Var key names are inferred from local `.env` at generation time; rerun `cf-typegen` after changing wrangler.jsonc bindings or `.env` keys.
