...
// Fail-closed flags (fallback false) require an explicit true; fail-open flags require an explicit false.
...
// node:https, not fetch, so Next's Data Cache can't serve a stale flag from a previous build.
...
// Called once per build from next.config.ts; any failure resolves to `fallback`.
...
// Only builds with UNDER_CONSTRUCTION_ENABLED=true evaluate the flag — preview/dev builds always render the store.
...
// Server-only: this module pulls in node:https for build-time evaluation.
