...
// the page. Static rotate-1 tilt + baked-in gentle float (bob + sway) via
// the `.sticker-float` utility — collapsed by the global reduced-motion
// guard. Purely decorative (aria-hidden).
//
...
export default function BouquetSticker({ className = '', size = 260 }: BouquetStickerProps) {
  return (
    <div
      aria-hidden="true"
    ...
    >
      <img
        ...
        height={size}
        draggable={false}
        className="h-full w-full rotate-1 [filter:drop-shadow(0_0_0_#fff)_drop-shadow(5px_0_0_#fff)_drop-shadow(-5px_0_0_#fff)_drop-shadow(0_5px_0_#fff)_drop-shadow(0_-5px_0_#fff)_drop-shadow(4px_4px_1px_#fff)_drop-shadow(-4px_4px_1px_#fff)_drop-shadow(4px_-4px_1px_#fff)_drop-shadow(-4px_-4px_1px_#fff)_drop-shadow(0_2px_4px_rgba(74,59,59,0.18))_drop-shadow(0_12px_20px_rgba(74,59,59,0.16))]"
      />
    </div>
  );
}